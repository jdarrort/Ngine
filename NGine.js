...
Element.prototype.html = function( in_html_content) {      this.innerHTML = in_html_content; }
    ...
    */
    if (isVarRE3.test(exp)) {
      ...
      exp = exp.replace( bracketRE, k => {
        ...
        // remove  '['  ']' 
        let x = k.replace(/[\[\]]+/g, function (k) {return "";})
        return "." + _this.processExpr(x, in_ctx, in_el, {}); // do not pass localContext, no binding to do on those sub processExpr
      });
      ...
      // }
      // toto, or toto.tata
      let i, o, var_membs = exp.split(".");
      for (i in var_membs) {
        o = var_membs[i];
        if (i == 0) {
          if (o == "#"){ // Refer to APP Context
            ref_obj = val = in_ctx.$app;
            continue;
          }
          if (o == "CST"  && window.CST) {  // Grab a CONSTANT Value
          ...
          let in$data = in_ctx.$data.find( cd => ( Object.keys(cd).indexOf(o) >= 0) );

          if (in_ctx.$render[o] !== undefined) { val = in_ctx.$render; }  // local rendering context (loop indexes, ...)
          //else if (in_ctx.$data[o] !== undefined) { val = in_ctx.$data; } 
          else if ( in$data !== undefined) { val = in$data; }
          else if (in_ctx.$app[o] !== undefined) { val = in_ctx.$app; }
          else {
