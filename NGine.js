...
Element.prototype.css = function (style, val) {
    if (typeof style === "object" && arguments.length == 1) {
        ...
        return this;
    } else if (arguments.length == 1) { return this.style[style] }
    this.style[style] = val;
    return this;
}
Element.prototype.prop = function (in_prop, val) {
    if (typeof in_prop === "object" && arguments.length == 1) {
        Object.entries(in_prop).forEach(([s, v]) => {
            this.setAttribute(s, v);
        })
        return this;
    } else if (arguments.length == 1) {
        return this.getAttribute(in_prop);
    } else {
        this.setAttribute(in_prop, val);
        return this;
    }
}
Element.prototype.attr = Element.prototype.prop;
...
var JNgine = new (function () {
    ...
    this.extend = function (in_app) {
        ...
        in_app.$forms = [];
        in_app.$errors = [];
        DOMrender = function (in_html_tpl_id, in_data, in_dom_obj) {
            this.$focus = null;
            try {
                if (in_dom_obj) {
                    in_dom_obj.clear()
                    return JNgine.RenderTemplateTo(in_html_tpl_id, this, in_data, in_dom_obj);
                } else {
                    return JNgine.RenderTemplate(in_html_tpl_id, this, in_data);
                }
            } catch (e) {
                this.err("DOMrender: " + e);
            }
        }
    }
    ...
    this.RenderTemplateTo = function (in_tpl_id, in_app, in_data, in_target_el) {
        ...
        if (in_target_el instanceof jQuery) {
            this.warn("Got some JQUERY there");
    ...
    this.RenderTemplate = function (in_tpl_id, in_app, in_data) {
        ...
        var result = this.StartRender(new_el, in_app, in_data, in_tpl_id);
        return { els: result.el.childNodes, ctx: result.ctx };
    }
    // ==================================================
    this.StartRender = function (el, in_app, in_ctx_data, in_from_template) {
    ...
    this.handleNode = function (in_el, in_ctx) {
        ...
        if (local_ctx["includetpl"]) {
            try {
                ...
                this.warn(`[${in_ctx.$template}] ` + in_msg);
                in_ctx.$errors.push({ el: in_el, txt: in_msg, tpl: in_ctx.$template });
            }
        } // end includetpl
...
JNgine.fn_map["if-defined"] = function (in_el, exp, ctx) {
    try {
        if (isDefined(this.processExpr(exp, ctx, in_el, { bindScope: C_NOBIND }))) {
            return true;
        }
        in_el.remove();
        return false;
...
JNgine.fn_map.tab_init = function (in_el, in_exp, in_ctx, in_lctx) {
    ...
    var tab_cfg = {
        ...
        title_els: [],
        content_els: [],
        addTabPanel: function (name, { title_el, content_el, cb_fn, cb_params }) {
            this.tab_panels[name] = {
                title_el: title_el,
                content_el: content_el,
                cb_fn: cb_fn,
                cb_params: cb_params,
                setContentEl: (el) => {
                    this.tab_panels[name].content_el = el;
                    this.addContentEl(el);
                    if ([name, "__SHOW_ALL__"].includes(this.getDefault())) {
                        el.classList.add("tab_body_inactive");
                    }
                }
            }

            if ([name, "__SHOW_ALL__"].includes(this.getDefault())) {
                title_el.classList.add(this.getStyle());
            }
            title_el.addEventListener('click', ev => {
                this.navTo(name, ev);
            });
            this.addTitleEl(title_el);

        },
        getTabPanel: function (name) { return this.tab_panels[name] },
        addTitleEl: function (el) { this.title_els.push(el) },
        addContentEl: function (el) { this.content_els.push(el) },
        getName: function () { return this.name; },
        setName: function (in_name) { this.name = in_name; },
        getDefault: function () { return this.default_active; },
        setDefault: function (in_default) { this.default_active = in_default; },
        getStyle: function () { return this.style; },
        setStyle: function (in_style) { this.style = in_style; },
        getCurrent: function () {
            return this.current_active;
        },
        // Do not use () =>  ! Use function() to preserve local this context
        navTo: function (in_tab_name, event) {
    ...
    try {
        ...
        if (isObjectRE.test(in_exp)) {
            ...
            if (typeof opt == "object") {
                tab_cfg.setName(opt.name);
                tab_cfg.setDefault(opt.default);
                tab_cfg.setStyle(opt.style);
                this.log("OK Object found");
            } else {
                tab_cfg.setName(in_ctx);
            }
        }
    } catch (e) {
        tab_cfg.setName(in_ctx);
...
JNgine.fn_map.tab_default_active = function (in_el, in_exp, in_ctx, in_lctx) {
    ...
    in_ctx.$cur_tab.setDefault(in_exp);
    return true;
}
...
JNgine.fn_map.tab_name = function (in_el, in_exp, in_ctx, in_lctx) {
    ...
    let $tab = in_ctx.$tabs[in_ctx.$cur_tab.getName()];
    ...
    if (!$tab.getTabPanel(t_name)) {
        $tab.addTabPanel(t_name, {
            title_el: in_el,
            cb_fn: typeof cb == "function" ? cb : null,
            cb_params: cb_params
        })
    }
...
JNgine.fn_map.ref_tab_name = function (in_el, in_exp, in_ctx, in_lctx) {
    ...
    in_el.d4_tab_group = in_ctx.$cur_tab.getName();
    in_el.d4_tab_name = t_name;

    let tab_panel = $tab.getTabPanel(t_name);
    if (!tab_panel) {
        return this.err(`TabPanel ${t_name} Not found on this context`);
    }
    tab_panel.setContentEl(in_el);
...
JNgine.fn_map.onoutsideclick = function (in_el, in_exp, in_ctx) {
    ...
    JNgine.closeOnOutsideClickMap.push({ el: in_el, ctx$app: in_ctx.$app, cb: function () { in_el.hide(); } });
