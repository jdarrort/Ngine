...
/*
    ...
    Inspired by Vue. Only for the rendering abilities.
     ...
     * Limited capability for bindings.
     * No Virtual Dom
...
Element.prototype.prop = function (in_prop, val) {
    if (val === undefined && typeof in_prop == "string") { return this.getAttribute(in_prop) };
...
**********************************************************************************/
/*
Process a DOM Tree, generally cloned from a <template> statement

Handling HTML Element nodes ATTRIBUTES
--------------------------------------
within the node definition, 3 specific tags for managing HTLM element attributes 
    $  ( variable/dynamic attributes ): means that the attribute value for that node is dynamic and will be interpreted once when rendering
        ex : <div $class="getSuitableClass()">
            ==> If 'getSuitableClass()' returns "myclass", will return <div class="myclass">

    $@  ( dynamic attributes, 2-way binding ): Same as "$", but 
           - the attribute of the HTML Elementvalue will automatically change when the bound variable changes
           - && the bound variable will change when the value of the HTML Element changes
        Mosstly used for forms element "INPUT", "SELECT", "TEXTAREA".
        ex : <input type="text"  $@value="firstname">
            assuming there is a $app.firstname attribute, or  $data.firstname attribute , inialized with 'Julien'
            ==> The rendering will return <input type="text" value="Julien">
            ==> If the user changes the value of that input to "Nelly" in the rendered page, $app.firstname will be updated to "Nelly"
            ==> Reciprocally, if a javascript code changes $app.firstname to "Heloise" ==> the input element will be updated with "Heloise"

        

    @  (event attribute) : to add custom eventListener to the element 
        Ex : <div @click="manageClickOnDiv()">
        ==> when that div will be clicked on, il will call the "manageClickOnDiv()" function, 
        Note that the function will be called having as "this" context the object hosting the function (which can be "$app" or the "$data" object) 


    :  (directives ): A directive is a function handling specific processing of an HTML Element
        There are many available directives (JNgine.fn_map [funtions])
        Ex1 : <div :handler="customManageThatElement(prop1,prop2)">
        ==> when processing that HTMLElement, the function customManageThatElement(prop1,prop2, el, ctx) will be called, 
              and will receive 2 (additional) attributes : the current EL, and the current rendering context.
        Ex2 : <span :if="active">Je suis Actif</span>
        ==> when processing that HTMLElement, if there is a non false/null/undefined value for $app, $data, $render, the DOM will be processed
             Otherwise, the HTMLElement will be skipped and not rendered.
        Note : 
        * PREDIRECTIVES : Some directives must be executed before the others 
        * POSTDIRECTIVES : Some directives are executed only after the child elements are already processed.

--------------------------------------
Handling Text content
--------------------------------------
Uses the {{}} (double open bracket) to specify a dynamic content
Ex :
   <span>TOTO</span>  ==> will be rendered as is
   <span>{{TOTO}}</span>  ==> will be rendered with the value of the property $app.TOTO, (or $data.TOTO) (or $render.TOTO)

--------------------------------------
Variables 
--------------------------------------
   <span>{{TOTO}}</span>   ==> Where should  TOTO  be defined ? 

The rendering process starts with JNgine.StartRender = function (el, in_app, in_data = {})


There are 4 sources of data managed , and they are looked upon in that order : 
 * $render : those are volatile variables positionned during the rendering process (by specific directives, such as ":for").
       Ex : <button :for="act in actions">({{act_idx}}) {{act}}</button>
        JNgine.StartRender(el,{actions : ["eat","drink","sleep"]},{})
        ==> Will produce "<button>(0) eat</button><button>(1) drink</button><button>(2) sleep</button>"


 * $data[] : is a stacked object containing providen data,  with $data[0] =  in_data
       Some directives will generate new custom data that will be stacked to $data while the HTMLElement and its children are processed

 * $app : is the providen in_app object (supposed to me the "service" instance calling the renderer )
                ==> properties are never altered through the rendering process
 
 * $instance_refs : another volatile variables positionned during the rendering process  (:ref).
       Ex : <button :ref="myCustomRefToThatEl">IAmAButton</button><span>MyRefTagName : '{{myCustomRefToThatEl.tagName}}' , containingText : '{{myCustomRefToThatEl.text()}}'</span>
        ==> Will produce  <div> <button>IAmAButton</button><span>MyRefTagName : 'BUTTON' , containingText : 'IAmAButton'</span>
        The directive ":ref" will add a property named "myCustomRefToThatEl" to $render, and with value = the HTMLElement
        When processing {{myCustomRefToThatEl.tagName}}, the property myCustomRefToThatEl of $render will be used.

 * Finally, a global object, (if defined) at window.CST (only if the expression starts with "CST")
        Ex <span>{{CST.myValue}}</span> 
        if window.CST is defined with  {myValue : "Coucou"}  ==> will return <span>Coucou</span> 

Is to possible to force usage of a specific data source with a prefix: 
    '$' ==> $data
    '#' ==> $app
    '@' ==> $instance_refs
    Ex : 
    JNgine.StartRender(el,{myValue : "IamAppValue"},{myValue : "IamDataValue"}) in  
      *  if el =  <span>{{$.myValue}}</span>   ==> Will return  <span>IamDataValue</span>
      *  if el =  <span>{{#.myValue}}</span>   ==> Will return  <span>IamAppValue</span>
 JNgine.StartRender(dom.create("div").html(` <span>{{#.myValue}}</span> `).appendTo(document.body),{myValue : "IamAppValue"},{myValue : "IamDataValue"})

* CST  : finally, global variables


  

// Limitations : !!! Achtung with <z> : Following example does not work :  When cloned, nodes within "<z>" are attached outside of the parent element ("<tr>")
/*
        <tr :pause>
            <th width="20"></th>
            <th>Params</th>
...
var JNgine = new (function () {
    ...
    */
    ...
    const PRE_DIRECTIVES = [":pause", ":if", ":if-defined", ":for", ":repeat", ":tabs_domain", ":default"];
    ...
    // ==================================================
    this.StartRender = function (el, in_app, in_data) {
        ...
        let ctx = {
            $app: in_app,
            //Not ready yet
            //$data: CST ? [{CST : CST}, in_data || {}] : [in_data || {}], // Stack context data, adding global "CST"
            $data: [in_data || {}], // Stack context data
            $render: {},    // Contains reference or data positionned while rendering (":for", ":setcontextel", ...), that might be required for following nodes.
            $focus: null,
            $root_el: el,
    ...
    this.handleNode = function (in_el, in_ctx) {
        ...
        try {
            ...
            let pre_directives = el_attrs.filter(a => PRE_DIRECTIVES.includes(a));
            ...
            // ------------------------------------
            // Those directives must be processed prior to the others
        ...
        // Une fois qu'on a fini de traiter un arbre, il peut y avoir des choses à faire
        try {
            // post Handler
            ...
            if (local_ctx["tabs_domain"]) {
                ...
                if (in_ctx.$cur_tab.default_active) {
                    in_ctx.$cur_tab.navTo(in_ctx.$cur_tab.default_active);
                }
                // If the tab was within a parent tab, restore it.
    ...
    this.processExpr = function (in_exp, in_ctx, in_el, in_lctx = { bindScope: C_NOBIND }) {
        ...
        const RE = /^([$]+|[#@]{1}|[a-zA-Z_][a-zA-Z_0-9]*)([\.\[\(]{0,1}.*)/
        ...
        if (root[0] == "$") {
            ...
            // Find Best context, in that order : $render, $data, $app, $instance_refs
    ...
    this.manageFormElem = function (in_el, in_ctx, in_local_ctx) {
        ...
        // Ajout d'une classe "mandatory" lorsque la valeur du champ est vide
        if (in_local_ctx.mandatory) {
            if (!in_el.value && in_el.value !== 0) {
                in_el.addClass("mandatory");
            }
            in_el.on("change", (e) => {
                if (!in_el.value && in_el.value !== 0) {
                    in_el.addClass("mandatory");
                } else {
                    in_el.removeClass("mandatory");
                }
            });
        }
    ...
    this.JBind = function (opt) {
        ...
        if (['INPUT', 'SELECT', 'TEXTAREA'].includes(opt.el.tagName)) {
            let fn = (e) => {
...
// Usage :
// :for="section in sections"   ('idx' available with  'section_idx' (stored at $render))
...
//----------------------------------------------------
JNgine.fn_map["addclass"] = function (in_el, in_exp, in_ctx, in_lctx) {
    let newval = this.processExpr(in_exp, in_ctx, in_el, in_lctx);
    if (newval) {
...
//----------------------------------------------------------------------
// Define the default active tab panel. Unless surrounded with {{]}}, considered as static panel name.
JNgine.fn_map.tab_default_active = function (in_el, in_exp, in_ctx, in_lctx) {
    let tmp = in_exp.match(/{{(.*)}}/);
    in_ctx.$cur_tab.default_active = tmp ? this.processExpr(tmp[1], in_ctx, in_el, in_lctx) : in_exp;
    return true;
}
...
JNgine.fn_map.tab_name = function (in_el, in_exp, in_ctx, in_lctx) {
    ...
    in_el.on('click', (e) => { $tab.navTo(t_name, e); });
    return true;
}
//-----------------------------------
//-----------------------------------
...
JNgine.fn_map.tab_nav_to = function (in_el, in_exp, in_ctx, in_lctx) {
    in_el.on('click', (e) => { in_ctx.$cur_tab.navTo(in_exp, e); });
    return true;
}
//----------------------------------------------------------------------
// add a callback when a tab is unselected
JNgine.fn_map.tab_on_leave = function (in_el, in_exp, in_ctx, in_lctx) {

    let $tab = in_ctx.$cur_tab;
    // Search current tab...
    try {
...
//----------------------------------------------------
// For Form elements
// adds an eventListener that will toggle a class if the element is empty or not
// (applies AFTER the element and sub tree is processed, cause we need to know the value)
JNgine.fn_map.mandatory = function (in_el, in_exp, in_ctx, in_lctx) {
    if (!['TEXTAREA', 'SELECT', 'INPUT'].includes(in_el.tagName)) {
        JNgine.warn(` WAR(mandatory) : Does not apply on ${in_el.tagName}\n Expr: ${in_exp}`, in_ctx)
    }
    in_lctx.mandatory = true;
    return true; // continue
}
